...
public class _01_IntroToQueue {
    public static void main(String[] args) {
    	...
    	for(int i = 0; i < 100; i++) {
    		myStack.push(new Random().nextDouble()*100);
    	}
    	...
    	String msg = "Removing " + randomNum + " elements from Queue: ";
    	DecimalFormat df = new DecimalFormat("##.##");
    	for(int i = 0; i < randomNum ;i++) {
    		msg += df.format(myQueue.remove()) + " ";
    	}
    	...
    	while(true) {
    		...
    		if(myQueue.size() < 5 && !myStack.isEmpty()) {
    			myQueue.add(myStack.pop());
    		}else if(myQueue.size() == 5 ) {
    			randomNum = new Random().nextInt(5) + 1;
    	    	msg = "Removing " + randomNum + " elements from Queue: ";
    	    	for(int i = 0; i < randomNum ;i++) {
    	    		msg += df.format(myQueue.remove()) + " ";
    	    	}
    	    	System.out.println(msg);
    		}else if  (myQueue.size() <= 5 && myStack.isEmpty() && !myQueue.isEmpty()) {
    			randomNum = new Random().nextInt(myQueue.size()+1);
    	    	msg = "Removing " + randomNum + " elements from Queue: ";
    	    	for(int i = 0; i < randomNum ;i++) {
    	    		msg += df.format(myQueue.remove()) + " ";
    	    	}
    		...
    		else if (myQueue.isEmpty() && myStack.isEmpty()){
    			System.out.println("Done with all elements");
    			System.exit(0);
    		}
    	}
